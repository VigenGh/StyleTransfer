...
van_gog=img.load_img(r"C:/Users/USER/Desktop/scream.jpg")
...
class style_transfer():
    ...
    def train(self,init_image,target_image,weights,num_iterations=10):
        n_iterations=1
        ...
        for train_step in range(num_iterations):
            ...
            if np.array(loss[0])<best_loss:
                ...
                best_img=np.array(init_image).copy()
            if n_iterations%1==0:
                end_time=time.time()
                i12=np.array(init_image).copy()
                images_per_iterations.append(i12)
